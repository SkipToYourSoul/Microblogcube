...
var isFirstTime = new Array();   //是否第一次选择
var currentWeibo = 0;			 //当前选择的微博
...
function handleJqueryData(data){
	...
	for (var key in nodes){                                                        
	    ...
	    //modify by xiafan
	    if (!existInOtherTree(key, currentRow)) {
		sigInst.addNode(key, node);
		nodeCount ++;
	    }

	    if(!nodeId[currentRow].contains(key))
		nodeId[currentRow].add(key);
	...
	for (var i = 0; i < data.rows.length; i++){
            ...
            edgeCount++;
	    edgeId[currentRow].push(data.rows[i][0]);
	}
    }//end if
    else{
	seeTheNodes();
...
function weiboChosen(param){
	...
	for(var x in nodeId)
	    if(nodeId[x].contains(weiboChosenMid)){
		isFirstTime[x]++;
		currentWeibo = x;
	    }
	if(hasThisWeibo){
...
function deleteNodes(){
    ...
    //for(var x in nodeId[temp])
    //sigInst.dropNode(nodeId[temp][x]);	
    sigInst.iterNodes(function(n){
       	if(nodeId[temp].contains(n.id) && !existInOtherTree(n.id, temp)){
	    n.hidden = 1;
	}
    });
