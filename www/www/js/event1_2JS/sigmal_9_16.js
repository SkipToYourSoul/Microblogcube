...
var testnode = Set.createNew(); // 当前sigInst中存在的点
...
function drawVisulization() {
	var url = "https://www.googleapis.com/fusiontables/v1/query?key="
			+ config.get("edge_index_key", "") + "&typed=false&sql=";
	var queryText = 'SELECT DocId, key FROM ' + config.get("edge_index_docid", "")
...
function findTable(mydata) {
	...
	var url = "https://www.googleapis.com/fusiontables/v1/query?key=" + row[1]
			+ "&typed=false&sql=";
	var len = selWeibo.keySet().length;
	var queryText = 'SELECT Retweetee,Retweeter,Time,RTMid FROM ' + row[0]
			+ ' WHERE RTMid IN (' + weiboChosenMid/* selWeibo.keySet()[len-1] */
			+ ');';
...
function drawControl(Data) {
	...
	control = new google.visualization.ControlWrapper({
		...
		'dataTable' : Data,
	// 'state': {'range': {'start': new Date(2011, 4, 4), 'end': new Date(2012,
	// 4, 5)}}
	});
	control.draw(Data);
...
function WeibohandleQueryResponse(response) {
	...
	for (i = 0; i < 7 & i < data.getNumberOfRows(); i++) {
		...
		hml += "<p class='weibo_info' onclick='weiboChosen(this);' mid='"
				+ data.getValue(i, 0) + "'>&nbsp;&nbsp;" + str + "</p></div>";
	}
...
function weiboChosen(param) {
	...
	// alert(content);
	if (isRunning && hasThisWeibo) {
		drawVisulization();
	} else if (isRunning && !hasThisWeibo) {
		// alert("delete!");
...
/*
 * existInOtherTree() 判断某个node是否存在于其他转发树
 */
function existInOtherTree(key, curId) {
	...
	for ( var i in nodeId) {
		if (i == curId)
			continue;
		if (nodeId[i].contains(key) && selWeibo.contains(nodeId[i].key)) {
			exist = true;
			break;
		}
	}
