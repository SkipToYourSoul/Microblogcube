...
function handleJqueryData(data){
    ...
    for (var key in nodes){                                                        //根据处理好的数据添加node
		...
		nodeCount ++;
		if(!nodeId[currentRow].hasOwnProperty(key))
			nodeId[currentRow].push(sigInst.getNodes(key));
		nodeId[6].push(sigInst.getNodes(key));
	}
...
function WeibohandleQueryResponse(response) {
	...
	nodeId[i] = new Array();
	nodeId[i].push(data.getValue(i,0));
	edgeId[i] = new Array();
...
function deleteNodes(){
	var temp = 0;
	for(var i=0;i<7;i++){
		if(nodeId[i][0]==weiboChosenMid)
			temp = i;
	}
	for(var x in nodeId[temp]){
		sigInst.dropNode(nodeId[temp][x].id);
		testnode.del(nodeId[temp][x].id);
		}
