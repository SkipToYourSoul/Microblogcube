...
var Set={
	...
	map.key = null;
        ...
        map.keySet=function(){
            var set =[];
            for (var key in this){
                if(typeof this[key]!="function" && key != "length"&& key !="key"){
                    set.push(key);
                }
            }
            return set;
...
function findTable(mydata){
	...
	var queryText = 'SELECT Retweetee,Retweeter,Time,RTMid FROM '+row[0]+' WHERE RTMid IN ('+weiboChosenMid/*selWeibo.keySet()[len-1]*/+')';
