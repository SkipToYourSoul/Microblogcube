...
//数据处理函数，用该函数从fusion table上获取数据
//The data from google
function drawVisualization_pre() {
    var queryText = 'SELECT Source, Target, Time FROM 1G99NUYnv8D-Fctmq91sE7GphdAZnUNODxkbU8kw';
...
//The data from jquery
/*function drawVisulization(){
    var url="https://www.googleapis.com/fusiontables/v1/query?sql=";
    var queryText = 'SELECT Source, Target,Time FROM 1G99NUYnv8D-Fctmq91sE7GphdAZnUNODxkbU8kw &key=AIzaSyDjilSHCLlgZGWoploUVrxA6tK8gi9oNCs';
    $.post(url+queryText, { },handleJqueryData);
    SetWeibo();
}*/

/*function drawVisulization(){
    var queryText = 'SELECT tableId FROM 1_JXKqAV-ugCFGp548AV4ygYtBeKDxekVowTcv2M WHERE eventId = 1';
    var query = new google.visualization.Query('http://www.google.com/fusiontables/gvizdata?tq='  + queryText);
    // Send the query with a callback function.
    query.send(findTable);
}*/
...
*/
    ...
    var queryText = 'SELECT Retweetee,Retweeter,Time FROM '+row[0]+' WHERE RTMid IN ('+selWeibo.keySet()[len-1]+')';

    if(isFirstTime){
        $.post(url+queryText, { },handleJqueryData);}
    else{
        $.post(url+queryText, { },handleJqueryData1);
    }
}
...
function handleJqueryData1(data){
