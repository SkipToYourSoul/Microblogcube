...
class ExSocialGraph(db.Model):
    ...
    edges = db.ListProperty(basestring, required=True, indexed=False)

def queryExSocialGraphByUid(uid):
    q = db.Query(ExSocialGraph).filter("uid = " , long(uid))
    return q.get()

def insertExSocialGraph(uid, friends, edges):
    record = queryExSocialGraphByUid(uid)
    if not(record is None):
        record.friends = friends
        record.edges = edges
    else:
        record = ExSocialGraph(uid=long(uid), friends=friends, edges=edges)
    record.put()
