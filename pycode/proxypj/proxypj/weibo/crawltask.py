...
class CrawlThread(threading.Thread):  
    ...
    def run(self):
        ...
        retryCount = 5
        retry = retryCount
        client1 = APIClient(app_key=settings.APP_KEY1, app_secret=settings.APP_SECRET1, redirect_uri=settings.CALLBACK_URL1)
...
class CrawlWorker(threading.Thread):
    ...
    def run(self):  # should run at most 1/s
        while True:
            if self.manager.stopWork:
                break
            try:
                uid = self.queue.get(True, 100)
            except Exception, e:
                continue
            logging.debug("start crawling uid:%s in the backend" % (str(uid)))
            """
            taskmanager.crawlTaskSuccess(uid)
            continue
            """
            ...
            while retry > 0:
                try:
                    user = client.users.show.get(uid=uid, source=settings.APP_KEY)
                    break
                except Exception, e : 
                    retry -= 1
            ...
            logging.debug("crawling friendship costs:%s s" % (end - start))             
